...
vector<double> portfolioGenerator::getDrawDown() const{
    vector<double> DD;
    double h = dailyVoP[0];
    for(int i = 1; i < dailyVoP.size(); i++){
        if(dailyVoP[i] >= h){
            DD.push_back(0);
            h = dailyVoP[i];
        } else {
            DD.push_back(dailyVoP[i]/h - 1);
        }
    }
    return DD;
}

void portfolioGenerator::getMDD() const{
    double result = 0;
    for(auto i:getDrawDown()){
        if(i < result) result = i;
    }
    cout << "The maximum draw-down is " << result << endl;
}

ostream& operator<<(ostream& os, const portfolioGenerator& p)
{
    vector<double> tmp = p.getDailyVoP();
    os << "The profit of this portfolio is\n$" << tmp.back() - tmp.front() << endl;
...
vector<vector<double>> portfolioGenerator::matrixM(vector<vector<double>> matrixA, vector<vector<double>> matrixB){
    vector<vector<double>> result;
    vector<double> tmp;
    ...
    for (int row = 0; row < matrixA.size(); row++){
        for (int col = 0; col < matrixB[0].size(); col++){
            double tt = 0;
            for(int i = 0; i < matrixB.size(); i++) tt += matrixA[row][i] * matrixB[i][col];
            tmp.push_back(tt);
        }
        result.push_back(tmp);
        tmp.clear();
...
vector<double> portfolioGenerator::matrixM(vector<vector<double>> matrixA, vector<double> verticalB){
    vector<double> result;
    for (int row = 0; row < matrixA.size(); row++){
        double tt = 0;
        for (int col = 0; col < verticalB.size(); col++){
            tt += matrixA[row][col] * verticalB[col];
        }
        result.push_back(tt);
    }
    return result;
}
vector<double> portfolioGenerator::matrixM(vector<double> horizontalA, vector<vector<double>> matrixB){
    vector<double> result;
    for (int col = 0; col < horizontalA.size(); col++){
        double tt = 0;
        for (int row = 0; row < matrixB.size(); row++){
            tt += horizontalA[row]* matrixB[row][col];
        }
        result.push_back(tt);
    }
    return result;
}
vector<vector<double>> portfolioGenerator::matrixM(vector<double> verticalA, vector<double> horizontalB){
    vector<vector<double>> result;
    for (int row = 0; row < verticalA.size(); row++){
        vector<double> tt;
        for (int col = 0; col < horizontalB.size(); col++) tt.push_back(verticalA[row]* horizontalB[col]);
        result.push_back(tt);
    }
    return result;
}
double portfolioGenerator::matrixMn(vector<double> horizontalA, vector<double> verticalB){
    double tt = 0;
    for (int i = 0; i < horizontalA.size(); i++) tt += horizontalA[i] * verticalB[i];
    return tt;
}
vector<double> portfolioGenerator::matrixM(vector<double> verticalA, double B){
    vector<double> result;

    for (int i = 0; i < verticalA.size(); i++){
        double tt = verticalA[i] * B;
        result.push_back(tt);
    }

    return result;
}
vector<vector<double>> portfolioGenerator::matrixS(vector<vector<double>> matrixA, vector<vector<double>> matrixB){
    vector<vector<double>> result;
    vector<double> tmp;

    for (int row = 0; row < matrixA.size(); row++){
        for (int col = 0; col < matrixB[0].size(); col++) tmp.push_back(matrixA[row][col] - matrixB[row][col]);
        result.push_back(tmp);
        tmp.clear();
    }
    return result;
